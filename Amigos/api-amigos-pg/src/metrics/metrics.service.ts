    ...
    const results = await this.repo.query(
      `
        ...
        SELECT 
          b.day,
          (
            (SELECT CAST(mt_value AS NUMERIC(30,6)) FROM ssr_amigos WHERE mt_name = 'SSR_AMIGOS--slave.totalizador' AND mt_time_2 = b.last_ts LIMIT 1)
            -
            (SELECT CAST(mt_value AS NUMERIC(30,6)) FROM ssr_amigos WHERE mt_name = 'SSR_AMIGOS--slave.totalizador' AND mt_time_2 = b.first_ts LIMIT 1)
          ) AS daily_value
        FROM bounds b
        ORDER BY b.day ASC
      `,
