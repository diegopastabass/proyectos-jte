...
@Injectable()
export class AraucariasService {
  constructor(
    ...
    const results = await this.repo.query(
      `
        WITH bounds AS (
          ...
          FROM araucarias
          WHERE mt_name = 'ARAUCARIAS--slave.totalizador'
        ...
        SELECT 
          b.day,
          (
            (SELECT CAST(mt_value AS NUMERIC(30,6)) FROM araucarias WHERE mt_name = 'ARAUCARIAS--slave.totalizador' AND mt_time_2 = b.last_ts LIMIT 1)
            -
            (SELECT CAST(mt_value AS NUMERIC(30,6)) FROM araucarias WHERE mt_name = 'ARAUCARIAS--slave.totalizador' AND mt_time_2 = b.first_ts LIMIT 1)
          ) AS daily_value
        FROM bounds b
        ORDER BY b.day ASC
      `,
  ...
  async getCaudal(dto: DateRangeDto): Promise<Metric[]> {
    ...
    if (hasRange) {
      ...
      const results = await this.repo.find({
        where: {
          mt_name: 'ARAUCARIAS--slave.caudal',
          mt_time_2: Between(start, end),
        },
        order: { mt_time_2: 'ASC' },
      });
    ...
    const results = await this.repo.find({
      where: { mt_name: 'ARAUCARIAS--slave.caudal' },
      order: { mt_time_2: 'DESC' },
      take: 300,
    });
