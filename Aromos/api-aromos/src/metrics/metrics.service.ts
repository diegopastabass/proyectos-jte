...
export class SsrAromosService {
    ...
    const results: DailyQueryResult[] = await this.repo.query(
      `
        WITH bounds AS (
          ...
          FROM ssr_aromos
          WHERE mt_name = 'SSR_AROMOS--slave.totalizador'
        ...
        FROM bounds b
        LEFT JOIN ssr_aromos s_first
          ON s_first.mt_name = 'SSR_AROMOS--slave.totalizador' AND s_first.mt_time_2 = b.first_ts
        LEFT JOIN ssr_aromos s_last
          ON s_last.mt_name = 'SSR_AROMOS--slave.totalizador' AND s_last.mt_time_2 = b.last_ts
        GROUP BY b.day
        ORDER BY b.day ASC
      `,
      [start, end],
    ...
    const results: DailyQueryResult[] = await this.repo.query(
      `
        WITH bounds AS (
          SELECT mt_time_2::DATE AS day, MIN(mt_time_2) AS first_ts, MAX(mt_time_2) AS last_ts
          FROM ssr_aromos
          WHERE mt_name = 'SSR_AROMOS--slave.horometro'
          AND mt_time_2 BETWEEN $1 AND $2
          GROUP BY mt_time_2::DATE
        )
        SELECT b.day,
        ...
        FROM bounds b
        LEFT JOIN ssr_aromos s_first
          ON s_first.mt_name = 'SSR_AROMOS--slave.horometro' AND s_first.mt_time_2 = b.first_ts
        LEFT JOIN ssr_aromos s_last
          ON s_last.mt_name = 'SSR_AROMOS--slave.horometro' AND s_last.mt_time_2 = b.last_ts
        GROUP BY b.day
        ORDER BY b.day ASC
      `,
      [start, end],
  ...
  async getNivel(dto: DateRangeDto): Promise<Metric[]> {
    ...
    const results = await this.repo.find({
      where: {
        mt_name: 'SSR_AROMOS--slave.estanque',
  ...
  async getCaudal(dto: DateRangeDto): Promise<Metric[]> {
    ...
    const results = await this.repo.find({
      where: {
        mt_name: 'SSR_AROMOS--slave.caudal',
        mt_time_2: Raw((alias) => `${alias} >= :start AND ${alias} < :end`, {
          start,
          end,
        }),
