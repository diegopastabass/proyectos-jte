...
export class SsrBoldosService {
  ...
  private async calculateAndCacheDaily(
    ...
    const cached = await this.repo.query(
      `SELECT mt_day, mt_value FROM ssr_boldos_daily_metrics WHERE mt_name = $1 AND mt_day BETWEEN $2 AND $3`,
    ...
    if (missingDates.length > 0) {
      ...
      for (const row of calculated) {
        ...
        await this.repo.query(
          `INSERT INTO ssr_boldos_daily_metrics (mt_name, mt_day, mt_value) 
           VALUES ($1, $2, $3) 
