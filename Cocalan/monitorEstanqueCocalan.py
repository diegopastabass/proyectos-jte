...
# Estado para alertas de desconexión (evitar spam)
alertas_desconexion_enviadas = {}
...
def enviar_alerta(mensaje: str, to: str = None):
    destinatario = to if to else TO
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    data = {"token": TOKEN, "to": destinatario, "body": mensaje}

    try:
        response = requests.post(API_URL, data=data, headers=headers, timeout=10)
        if response.status_code == 200:
            logging.info(f"📤 Alerta enviada con éxito a {destinatario}: {mensaje}")
        else:
...
def procesar_estanque(nombre_publico, nivel_alerta):
    ...
    # --- Verificación de Desconexión (Staleness) ---
