...
let MirafloresService = class MirafloresService {
    ...
    async getHorometro(dto) {
        ...
        SELECT 
          b.day,
          (
            (SELECT CAST(mt_value AS NUMERIC(30,6)) FROM pozo_miraflores WHERE mt_name = 'POZO_MIRAFLORES--slave.horometro' AND mt_time_2 = b.last_ts LIMIT 1)
            -
            (SELECT CAST(mt_value AS NUMERIC(30,6)) FROM pozo_miraflores WHERE mt_name = 'POZO_MIRAFLORES--slave.horometro' AND mt_time_2 = b.first_ts LIMIT 1)
          ) AS daily_value
        FROM bounds b
        ORDER BY b.day ASC
      `, [start, end]);
