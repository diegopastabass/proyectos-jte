...
export class MirafloresService {
  ...
  async getTotalizador(dto: DateRangeDto): Promise<Metric[]> {
    ...
    // Usamos subconsultas con LIMIT 1 para evitar duplicados en el JOIN
    const results = await this.repo.query(
      `
        WITH bounds AS (
          SELECT 
    ...
    return results.map((row: any) => {
      // Aseguramos que 'day' sea un string manejable
      ...
      return {
        time: dateStr,
        value: Number(row.daily_value || 0), // Manejo de nulos por seguridad
      };
    });
  }

  // getHorometro (Copia de la lógica corregida)
