...
export class MetricsService {
  ...
  async getViverosDataForExcel(
    fechaInicio: string,
    fechaFin: string,
  ): Promise<{ id: number; sensor_id: number; value: number; time: Date }[]> {
    try {
      const startDateTime = new Date(`${fechaInicio}T00:00:00`);
      const endDateTime = new Date(`${fechaFin}T23:59:59`);

      const results: MetricsRow[] = await this.metricRepository.query(
        `
  ...
  async getMinMax(
    ...
    try {
      ...
      this.logger.error('Error retrieving min/max metrics', error);
      throw new InternalServerErrorException(
