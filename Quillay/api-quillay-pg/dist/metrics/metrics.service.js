...
exports.SsrQuillayService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const metrics_entity_1 = require("./models/metrics.entity");
let SsrQuillayService = class SsrQuillayService {
    repo;
    constructor(repo) {
        this.repo = repo;
    }
  ...
  `);
        ...
        const hormigon = await calcularTiempoVaciado('SSR_QUILLAY--slave.estanque_hormigon');
        const metalico = await calcularTiempoVaciado('SSR_QUILLAY--slave.estanque_metalico');
        return {
            snapshot,
    ...
    async getTotalizador(dto) {
        ...
        WITH bounds AS (
          ...
          FROM ssr_quillay
          WHERE mt_name = 'SSR_QUILLAY--slave.totalizador'
        ...
        LEFT JOIN ssr_quillay s_first
          ON s_first.mt_name = 'SSR_QUILLAY--slave.totalizador' AND s_first.mt_time_2 = b.first_ts
        LEFT JOIN ssr_quillay s_last
          ON s_last.mt_name = 'SSR_QUILLAY--slave.totalizador' AND s_last.mt_time_2 = b.last_ts
        GROUP BY b.day
        ORDER BY b.day ASC
      `, [start, end]);
    ...
    async getHorometro(dto) {
        ...
        WITH bounds AS (
          SELECT mt_time_2::DATE AS day, MIN(mt_time_2) AS first_ts, MAX(mt_time_2) AS last_ts
          FROM ssr_quillay
          WHERE mt_name = 'SSR_QUILLAY--slave.horometro'
          AND mt_time_2 BETWEEN $1 AND $2
          GROUP BY mt_time_2::DATE
        )
        SELECT b.day,
        ...
        FROM bounds b
        LEFT JOIN ssr_quillay s_first
          ON s_first.mt_name = 'SSR_QUILLAY--slave.horometro' AND s_first.mt_time_2 = b.first_ts
        LEFT JOIN ssr_quillay s_last
          ON s_last.mt_name = 'SSR_QUILLAY--slave.horometro' AND s_last.mt_time_2 = b.last_ts
        GROUP BY b.day
        ORDER BY b.day ASC
      `, [start, end]);
        return results.map((row) => ({
...
exports.SsrQuillayService = SsrQuillayService;
exports.SsrQuillayService = SsrQuillayService = __decorate([
