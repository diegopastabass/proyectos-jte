...
export class SsrQuillayService {
  ...
  async getTotalizador(dto: DateRangeDto): Promise<Metric[]> {
    ...
    const results: DailyQueryResult[] = await this.repo.query(
      `
        ...
        SELECT b.day,
          (MAX(CAST(s_last.mt_value AS DECIMAL(30,6))) - MIN(CAST(s_first.mt_value AS DECIMAL(30,6)))) AS daily_value
        FROM bounds b
  ...
  // Métodos restantes sin cambios (getNivel, getNivel2, getCaudal)...
