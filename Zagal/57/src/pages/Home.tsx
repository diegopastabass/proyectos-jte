...
function Home() {
    ...
    const fetchData = async () => {
      try {
        const [metricRes, intervalRes, updateRes] = await Promise.all([
          fetch("https://app.jteanalytics.cl/zagal/57/metrics"),
          fetch("https://app.jteanalytics.cl/zagal/57/interval"),
          fetch("https://app.jteanalytics.cl/zagal/57/update"),
        ]);
  ...
  const parcelas = Array.from(new Set(Object.values(parcelaMap)));
            ...
            <div className="mb-3 mx-3">
              ...
              >
                {parcelas.map((parcela, index) => (
                  <option key={index} value={parcela}>
                    {parcela}
                  </option>
                ))}
              </select>
