...
export class MetricsService {
  ...
  async findLatestByName(name: string): Promise<Metric | null> {
    try {
      const [rows] = await this.dbConfig.pool.query(
        "SELECT mt_id, mt_name, mt_value, mt_time_2 FROM parc_57_zagal WHERE mt_name = ? ORDER BY mt_time_2 DESC LIMIT 1",
        [name]
      );
  ...
  async findLastUpdateTime(): Promise<Date | null> {
    try {
      const [rows] = await this.dbConfig.pool.query(
        "SELECT MAX(mt_time_2) AS last_update FROM parc_57_zagal"
      );
    ...
    try {
      ...
      WITH ordered AS (
          ...
          WHERE mt_name IN (
              ...
              'PARC_57_ZAGAL--slave.sector8',
              'PARC_57_ZAGAL--slave.sector9'
          )
            AND mt_time_2 >= ?
