...
export class MetricsService {
  ...
  async findLatest(): Promise<LatestMetricResult> {
      ...
      FROM
          (
              ...
              FROM
                  parc_82_zagal
              WHERE
                  mt_name IN ('PARC_82_ZAGAL--slave.caudal', 'PARC_82_ZAGAL--slave.totalizador')
          ) AS sub
      WHERE
          sub.rn = 1;
    `;
    ...
    try {
      ...
      for (const row of metrics) {
        ...
        if (row.mt_name === "PARC_82_ZAGAL--slave.caudal") {
          result.caudal = value;
        } else if (row.mt_name === "PARC_82_ZAGAL--slave.totalizador") {
  ...
  async findLatestByName(name: string): Promise<Metric | null> {
    try {
      const [rows] = await this.dbConfig.pool.query(
        "SELECT mt_id, mt_name, mt_value, mt_time_2 FROM parc_82_zagal WHERE mt_name = ? ORDER BY mt_time_2 DESC LIMIT 1",
        [name]
      );
  ...
  async findLastUpdateTime(): Promise<Date | null> {
    try {
      const [rows] = await this.dbConfig.pool.query(
        "SELECT MAX(mt_time_2) AS last_update FROM parc_82_zagal"
      );
  ...
  > {
    try {
      ...
      WITH ordered AS (
          ...
          FROM parc_82_zagal
          WHERE mt_name IN (
              'PARC_82_ZAGAL--slave.sector1',
              'PARC_82_ZAGAL--slave.sector2',
              'PARC_82_ZAGAL--slave.sector3',
              'PARC_82_ZAGAL--slave.sector4',
              'PARC_82_ZAGAL--slave.sector5',
              'PARC_82_ZAGAL--slave.sector6',
              'PARC_82_ZAGAL--slave.sector7',
              'PARC_82_ZAGAL--slave.sector8',
              'PARC_82_ZAGAL--slave.sector9',
              'PARC_82_ZAGAL--slave.sector10',
              'PARC_82_ZAGAL--slave.sector11',
              'PARC_82_ZAGAL--slave.sector12'
          )
            AND mt_time_2 >= ?
          ...
          SELECT 
              ...
              COALESCE(
                (
                  ...
                  WHERE c.mt_name = 'PARC_82_ZAGAL--slave.caudal'
