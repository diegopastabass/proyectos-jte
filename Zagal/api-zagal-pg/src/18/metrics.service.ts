...
export class MetricsService {
  ...
  private sectorAreas: Record<string, number> = {
    "PARC_18_ZAGAL--slave.sector1": 76.160,
    "PARC_18_ZAGAL--slave.sector2": 69.308,
    "PARC_18_ZAGAL--slave.sector3": 70.692,
    "PARC_18_ZAGAL--slave.sector4": 64.430,
  ...
  async getDailyIrrigationSummary(startDate: string, endDate: string) {
    try {
      // Query refactorizada para Postgres (ver explicación previa)
      const query = `
      WITH ordered AS (
          SELECT 
      ...
      return rows.map((r) => ({
        ...
        date: r.date, // TypeORM suele devolver string YYYY-MM-DD o Date object
