// 57/services/metrics.service.ts
...
export class MetricsService {
  constructor(
    @InjectRepository(Parc57Zagal)
    private readonly repo: Repository<Parc57Zagal>
  ) {}
  ...
  async findLatest(): Promise<LatestMetricResult> {
    try {
      const rows = await this.repo.query(`
        ...
        FROM parc_57_zagal
        WHERE mt_name IN ('PARC_57_ZAGAL--slave.caudal', 'PARC_57_ZAGAL--slave.totalizador')
      ...
      for (const row of rows) {
        ...
        if (row.mt_name.includes('totalizador')) result.totalizador = value / 10;
      }
  ...
  async findLatestByName(name: string): Promise<Parc57Zagal | null> {
    try {
      ...
      throw new InternalServerErrorException("Error fetching latest metric by name");
    }
  }
  ...
  async findIrrigationInterval() {
    try {
      ...
      throw new InternalServerErrorException("Error fetching irrigation interval");
    }
  }
    ...
    try {
      ...
      caudal_avg AS (
         ...
         (SELECT AVG(NULLIF(c.mt_value, '')::numeric) 
          ...
          WHERE c.mt_name = 'PARC_57_ZAGAL--slave.caudal' AND c.mt_time_2 BETWEEN i.inicio AND i.fin
         ) as caudal
         FROM intervalos i
      )
      SELECT 
      ...
      throw new InternalServerErrorException("Error fetching irrigation summary");
    }
  }
}