    ...
    const results: DailyQueryResult[] = await this.repo.query(
      `
        WITH bounds AS (
          ...
          FROM ssr_zuniga
          WHERE mt_name = 'SSR_ZUNIGA--slave.totalizador'
        ...
        LEFT JOIN ssr_zuniga s_first
          ON s_first.mt_name = 'SSR_ZUNIGA--slave.totalizador' AND s_first.mt_time_2 = b.first_ts
        LEFT JOIN ssr_zuniga s_last
          ON s_last.mt_name = 'SSR_ZUNIGA--slave.totalizador' AND s_last.mt_time_2 = b.last_ts
        GROUP BY b.day
        ORDER BY b.day ASC
      `,
      [start, end],
    ...
    const results: DailyQueryResult[] = await this.repo.query(
      `
        WITH bounds AS (
          SELECT mt_time_2::DATE AS day, MIN(mt_time_2) AS first_ts, MAX(mt_time_2) AS last_ts
          FROM ssr_zuniga
          WHERE mt_name = 'SSR_ZUNIGA--slave.horometro'
          AND mt_time_2 BETWEEN $1 AND $2
          GROUP BY mt_time_2::DATE
        )
        SELECT b.day,
        ...
        FROM bounds b
        LEFT JOIN ssr_zuniga s_first
          ON s_first.mt_name = 'SSR_ZUNIGA--slave.horometro' AND s_first.mt_time_2 = b.first_ts
        LEFT JOIN ssr_zuniga s_last
          ON s_last.mt_name = 'SSR_ZUNIGA--slave.horometro' AND s_last.mt_time_2 = b.last_ts
        GROUP BY b.day
        ORDER BY b.day ASC
      `,
      [start, end],
